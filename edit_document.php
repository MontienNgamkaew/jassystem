...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'update') {
    ...
    try {
        ...
        $stmt = $pdo->prepare("UPDATE documents SET date=?, customer_id=?, company_id=?, total_amount=?, include_vat=?, show_date=?, note=? WHERE id=?");
        $stmt->execute([$date, $customer_id, $company_id, $grand_total, $include_vat, $show_date, $note ?: null, $id]);
                        ...
                        <div class="row align-items-center">
                            <div class="col-md-8">
                                ...
                                <div class="mt-3">
                                    ...
                                    <textarea class="form-control form-control-sm" name="doc_note" id="docNote" rows="3" placeholder="เช่น รับประกันสินค้า 1 ปี / ราคานี้รวมค่าติดตั้งแล้ว"><?= htmlspecialchars($doc['note'] ?? '') ?></textarea>
                                </div>
                            </div>
